...
// Для всех разное
const DATA_DIR = "./data";
const serverConfig = require(`./data/config.json`);
const SYSTEM_ID = "g6zyv8tj53w28ov7cl";
...
const BASE_URL = "https://ru-academy.online";

// Сертификаты / Для всех одни
...
const SIGNER_KEY_PASSPHRASE =
    serverConfig.appleCredentials.SIGNER_KEY_PASSPHRASE || "N/A";
const AUTH_KEY_PATH = `${DATA_DIR}/certs/authkey.p8`;


const NOTIFICATION_PLACEHOLDER = "Здесь будут уведомления";
...
const apnProvider = new apn.Provider({
    ...
    token: {
        key: AUTH_KEY_PATH,
        keyId: serverConfig.appleCredentials.APPLE_AUTH_KEY_ID,
        teamId: serverConfig.appleCredentials.TEAM_IDENTIFIER,
    },
});
...
// `
//     IF EXISTS(SELECT * FROM notifications WHERE user_id='${userId}')
...
//     IIF(EXISTS(SELECT * FROM notifications WHERE user_id='${userId}'), UPDATE notifications SET pushToken='${pushToken}' WHERE user_id='${userId}', INSERT OR REPLACE INTO notifications (user_id, pushToken) VALUES ('${userId}', '${pushToken}'));
// `
...
function setClient(userId, client) {
    db.run(`
        INSERT OR REPLACE INTO notifications (user_id, client)
...
const getClient = (userId) => getField(userId, "client");
...
async function createNotificationPass(urlRoot, userId, notificationText) {
    ...
    try {
        ...
        const passBody = JSON.stringify({
            formatVersion: 1,
            webServiceURL: `${urlRoot}/apns/api/webhooks/wallet/${userId}`,
            ...
            passTypeIdentifier: serverConfig.appleCredentials.PASS_TYPE_IDENTIFIER,
            serialNumber: userId,
            teamIdentifier: serverConfig.appleCredentials.TEAM_IDENTIFIER,
            ...
            "barcode" : {
                altText: String(accountData.CardNumber),
                "message" : String(accountData.CardNumber),
...
async function sendNotification(userId, text) {
    ...
    if (pushToken) {
        ...
        notification.topic = serverConfig.appleCredentials.PASS_TYPE_IDENTIFIER;
...
// const getHost = (req) => {
//     // return (req.headers["x-forwarded-proto"] || req.protocol) + "://" + (req.headers["x-forwarded-host"] || req.headers.host);
//     return "https://ru-academy.online";
// }
...
async function getAccount(token) {
    ...
    const response = await fetch('https://customerapi2.mi.crm4retail.ru/json.rpc/', {
        method: 'POST',
        body: JSON.stringify(body),
        headers: {'Content-Type': 'application/json-rpc'}
    });
    const data = await response.json();
    return data.result.Cards[0];
}
...
async function getUserData(token) {
    const body = {
        ...
        params: [SYSTEM_ID, token]
    };

    const response = await fetch('https://testauth.crm4retail.ru/tnt', {
        method: 'POST',
        body: JSON.stringify(body),
        headers: {'Content-Type': 'application/json-rpc'}
    });
...
router.get("/apns/api/checkUser/:userId", auth, async (req, res) => {
    const {userId} = req.params;
    const accountData = await checkUser(userId)
    res.send(accountData);
});

router.get("/apns/api/client/:client/passUrl/:userId", auth, async (req, res) => {
    const {userId, client} = req.params;
    res.send({
        url: `${BASE_URL}/apns/api/client/${client}/pass/${encrypt(userId, serverConfig.SECRET)}.pkpass`,
    });
});

router.post("/apns/api/sendNotification/:userId", auth, async (req, res) => {
    const {userId} = req.params;
    let { isUpadteCard, text} = req.body;
    try {
        if (isUpadteCard) text = await getNotificationText(userId)
...
router.get("/apns/api/client/:client/pass/:encryptedUserId.pkpass", async (req, res) => {
    const {encryptedUserId, client} = req.params;
    let userId;
    try {
        userId = decrypt(encryptedUserId, serverConfig.SECRET);
    } catch (e) {
        res.sendStatus(400);
        throw e;
    }
    ...
    setNotificationText(userId, NOTIFICATION_PLACEHOLDER)
...
router.get(
    "/apns/api/webhooks/wallet/:userId/v1/devices/:deviceId/registrations/:passTypeId",
    (req, res) => {
        const {userId} = req.params;
        res.send({
...
router.get(
    "/apns/api/webhooks/wallet/:userId/v1/passes/:passTypeId/:serialNumber",
    appleAuth,
    async (req, res) => {
        const {userId} = req.params;
        const host = BASE_URL;
...
router.post("/apns/api/webhooks/wallet/:serialNumber/v1/log", appleAuth, (req, res) => {
    console.error(req.body);
    res.send({});
});

router.post(
    "/apns/api/webhooks/wallet/:userId/v1/devices/:deviceId/registrations/:passTypeId/:serialNumber",
    appleAuth,
    (req, res) => {
        const {userId} = req.params;
