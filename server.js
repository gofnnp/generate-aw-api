...
router.get("/apns/api/test/:userId", auth, async (req, res) => {
    const {userId} = req.params;
    const accountData = await getUserData(userId)
    res.send(accountData);
});

router.get("/apns/api/passUrl/:userId", auth, async (req, res) => {
    const {userId} = req.params;
    res.send({
        url: `${getHost(req)}/apns/api/pass/${encrypt(userId, config.SECRET)}.pkpass`,
    });
});
...
router.get("/apns/api/pass/:encryptedUserId.pkpass", async (req, res) => {
